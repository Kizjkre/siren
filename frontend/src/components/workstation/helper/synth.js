...
export const play = (data, globalSettings) => {
  if (context) {
    ...
    let key = globalSettings.key === 'none' ? 'C' : globalSettings.key;
    ...
    data.forEach(d => {
      if (d.settings.channel.length === 0) {
        ...
        gain.gain.value = d.settings.volume / 100 / data.length;

        const max = Math.max(...d.data);
        const min = Math.min(...d.data);
        const num = SCALES[key].length * 2; // TODO: This is incorrect
